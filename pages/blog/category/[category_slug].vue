...
const route = useRoute();const { findOne, find } = useStrapi();
...
const { data: pageData } = await useStrapiAsyncData(
  `blog-category${locale.value}`,
  () => findOne('blog-page', { locale: locale.value })
);

const { data: category } = await useStrapiAsyncDataFirst(
  `category-${locale.value}-${category_slug}`,
  () => useStrapi().findOne('categories', { where: { slug: category_slug }, locale: locale.value })
);
        ...
        <aside class="w-full md:w-64 flex-shrink-0">
          <div class="space-y-6">
            ...
            <div v-if="relatedCategories">
              <h3 class="text-lg font-bold mb-2">Other Categories</h3>
